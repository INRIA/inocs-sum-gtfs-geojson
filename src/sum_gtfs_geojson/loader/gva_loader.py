...
class GenevaLoader(AbstractLoader):
    def load_stops(self):
        ...
        public_transport_stations = public_transport_stations.where(pd.notnull(public_transport_stations), None)
        stops = []
        for _, row in public_transport_stations.iterrows():
            try:
                ...
                continue
        print("Success lines to process / total lines : ", len(stops), "/", len(public_transport_stations))
        return stops
    ...
    def load_routes(self):
        ...
        public_transport_routes = public_transport_routes.where(pd.notnull(public_transport_routes), None)
        routes = []
            ...
            try:
                routes.append(
                    Route(
                        ...
                        route_text_color=safe_get(row, "route_text_color", None),
                    )
                )
            except ValidationError as e:
                #logger.exception(f"ValidationError in load_routes: {e}")
                continue
        print("Success lines to process / total lines : ", len(routes), "/", len(public_transport_routes))
        return routes
    ...
    def load_trips(self):
        ...
        public_transport_trips = public_transport_trips.where(pd.notnull(public_transport_trips), None)
            ...
            try:
                trips.append(
                    Trip(
                        ...
                        trip_headsign=safe_get(row, "trip_headsign", None, str),
                        trip_short_name=safe_get(row, "trip_short_name", None, str),
                ...
                continue
        print("Success lines to process / total lines : ", len(trips), "/", len(public_transport_trips))
        return trips
    ...
    def load_stop_times(self):
        ...
        public_transport_stop_times = public_transport_stop_times.where(pd.notnull(public_transport_stop_times), None)
        print("Stop times lines to process: ", len(public_transport_stop_times))
            ...
            try:
                stop_times.append(
                    StopTime(
                        ...
                        departure_time=safe_get(row, "departure_time", "", str),
                        stop_id=safe_get(row, "stop_id", "", str),
                        stop_sequence=safe_get(row, "stop_sequence", 0, int),
                        stop_headsign=safe_get(row, "stop_headsign", None, str),
                        pickup_type=safe_get(row, "pickup_type", None, int),
                        drop_off_type=safe_get(row, "drop_off_type", None, int),
                        shape_dist_traveled=safe_get(row, "shape_dist_traveled", None, float),
                ...
                continue
        print("Success lines to process / total lines : ", len(stop_times), "/", len(public_transport_stop_times))
        return stop_times
    ...
    def load_bike_stations(self):
        ...
        shared_bikes_stations = shared_bikes_stations.where(pd.notnull(shared_bikes_stations), None)
        bike_stations = []
        for _, row in shared_bikes_stations.iterrows():
            try:
                ...
                continue
        print("Success lines to process / total lines : ", len(bike_stations), "/", len(shared_bikes_stations))
    ...
    def load_ridership(self):
        ...
        public_transport_ridership = public_transport_ridership.where(pd.notnull(public_transport_ridership), None)
        ridership_data = []
            ...
            try:
                ridership_data.append(
                    Ridership(
                        ...
                        boardings=safe_get(row, "Number of Boarding Passengers", 0, int),
                        alightings=safe_get(row, "Number of Disembarking Passengers", 0, int),
                ...
                continue
        print("Success lines to process / total lines : ", len(ridership_data), "/", len(public_transport_ridership))
            ...
            try:
                bike_trips_data.append(
                    BikeTrip(
                        ...
                        trip_started_at_utc=safe_get(row, "trip_started_at_utc", "", str),
                        trip_ended_at_utc=safe_get(row, "trip_ended_at_utc", "", str),
                        ...
                        distance_in_km=safe_get(row, "distance_in_km", None, float),
                    )
                )
            except ValidationError as e:
                logger.exception(f"ValidationError in load_bike_trips for line {row.index}: {e}")
                continue
        print("Success lines to process / total lines : ", len(bike_trips_data), "/", len(bike_trips))
